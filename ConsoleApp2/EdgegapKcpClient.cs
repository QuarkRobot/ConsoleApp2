        ...
        protected override bool RawReceive(out ArraySegment<byte> segment)
        {
            ...
            try
            {
                if (socket.ReceiveNonBlocking(relayReceiveBuffer, out ArraySegment<byte> content))
                {
                    ...
                    {
                        Debug.Print($"EdgegapClient: message of {content.Count} is too small to parse.");
                        return false;
                    }
                        ...
                        case (byte)MessageType.Ping:
                            {
                                ...
                                if (connectionState != last) Debug.Print($"EdgegapClient: state updated to: {connectionState}");
