...
namespace ConsoleApp2
{
    public class EdgegapKcpServer : KcpServer
    {
        ...
        protected override bool RawReceiveFrom(out ArraySegment<byte> segment, out int connectionId)
        {
            ...
            try
            {
                ...
                if (relaySocket.ReceiveNonBlocking(relayReceiveBuffer, out ArraySegment<byte> content))
                {
                    ...
                    {
                        Debug.Print($"EdgegapServer: message of {content.Count} is too small to parse header.");
                        return false;
                    }
                        ...
                        case (byte)MessageType.Ping:
                            {
                                ...
                                if (state != last) Debug.Print($"EdgegapServer: state updated to: {state}");
                        ...
                        case (byte)MessageType.Data:
                            {
                                ...
                                if (content.Count <= 5)
                                {
                                    Debug.Print($"EdgegapServer: message of {content.Count} is too small to parse connId.");
                                    return false;
                                }
        ...
        void SendPing()
        {
            ...
            {
                Debug.Print($"EdgegapServer: failed to ping. perhaps the relay isn't running? {e}");
            }
        }
